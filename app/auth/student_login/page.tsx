...
// 👇 Define expected API response shape for type safety
...
export default function StudentLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 👇 Use type assertion so TypeScript knows the response shape
      const res = await authService.login({
        email: form.email,
        password: form.password,
      });

      // 🛠️ FIX: access token via res.data, not directly on res
      ...
      const studentData = {
        name: res.data.user?.name || (res as any).student?.name || "Student",
        email: res.data.user?.email || (res as any).student?.email || form.email,
        id: res.data.user?.id || (res as any).student?.id,
        role: 'student'
      };
